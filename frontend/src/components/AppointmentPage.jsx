...
const AppointmentPage = () => {
  ...
  const loadDoctorAppointments = useCallback(async () => {
    ...
    // Headers for /api/appointments/me
    const headers = {
      "Content-Type": "application/json",
    };

    // If we have a Clerk token, add it to the headers
    if (token) {
    ...
    try {
      const resp = await fetch(`${API_BASE}/api/appointments/me`, {
        method: "GET",
        headers,
      });

      if (!resp.ok) {
        throw new Error(`Failed to fetch appointments (status ${resp.status})`);
      }
      ...
      console.error("Error calling /api/appointments/me:", err.message || err);
      ...
      if (user?.id) {
        try {
          ...
          if (!debugResp.ok) {
            throw new Error(
              `Failed to fetch appointments (status ${debugResp.status})`,
            );
          }

          console.log("Debug fallback response:", debugResp);

          const debugData = await debugResp.json();

          const fetched =
            debugData?.appointments ?? debugData?.data ?? debugData ?? [];
  ...
  const loadServiceAppointments = useCallback(async () => {
    ...
    // Headers for /api/service-appointments/me
    const headers = {
      "Content-Type": "application/json",
    };
    ...
    try {
      const resp = await fetch(`${API_BASE}/api/service-appointments/me`, {
        method: "GET",
        headers,
      });
      ...
      console.log("Response from /api/service-appointments/me:", resp);
      ...
      if (user?.id) {
        try {
          ...
          console.log("Debug fallback response (services):", debugResp);
  ...
  return (
    <div className={`${aps.pageContainer}`}>
      ...
      <div className={`${aps.maxWidthContainer}`}>
        <h1 className={`${aps.doctorTitle}`}>Your Doctor Appointments</h1>
        {loadingDoctors && (
          <div className={`${aps.loadingText}`}>Loading Doctor...</div>
        )}
        {!loadingDoctors && appointmentData.length === 0 && (
          <div className={`${aps.emptyStateText}`}>
          ...
          {appointmentData.map((item) => (
            <div key={item.id} className={`${cardStyles.doctorCard}`}>
              <div className={`${cardStyles.doctorImageContainer}`}>
                <img
                  src={item.image || "/doctor_image.jpg"}
                  alt={item.doctor}
              ...
              <p className={`${cardStyles.dateContainer}`}>
                <Calendar className={`${iconSize.medium}`} /> {item.date}
              </p>
        ...
        <h2 className={`${aps.serviceTitle}`}>Your Booked Services</h2>
        {loadingServices && (
          <div className={`${aps.serviceLoadingText}`}>
            Loading Service Bookings...
          </div>
        )}
        {!loadingServices && serviceData.length === 0 && (
          <div className={`${aps.emptyStateText}`}>No services found...</div>
        )}
