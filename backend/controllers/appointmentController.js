...
// THIS FUNCTION ENSURES THAT THE USER IS COMING FROM CLERK, RETURN USER DETAILS
...
export const getAppointments = async (req, res) => {
  try {
    ...
    if (patientClerkId) filter.patientClerkId = patientClerkId;
...
export const getAppointmentsByPatient = async (req, res) => {
  try {
    ...
    // console.log("resolvedCreatedBy query:", resolvedCreatedBy);
  ...
  try {
    ...
    if (existingBooking) {
      return res.status(409).json({
        success: false,
        message: "You have already made a booking for this slot...",
      });
    }
    ...
    if (!resolvedOwner) {
      resolvedOwner = MAJOR_ADMIN_ID || String(doctorId);
    }
    ...
    const parsedAge = age !== "" && age !== undefined ? Number(age) : undefined;
    ...
    const base = {
      ...
      notes: notes || "",
      createdBy: clerkUserId, // For the particular user
      owner: resolvedOwner,
      sessionId: null,
    };
  ...
  try {
    ...
    if (!appt) {
      ...
      if (meta.doctorId && meta.mobile && meta.patientName) {
        if (!mongoose.Types.ObjectId.isValid(meta.doctorId)) {
          return res.status(400).json({
            success: false,
            message: "Invalid doctor metadata",
          });
        }

        appt = await Appointment.findOneAndUpdate(
          {
            doctorId: new mongoose.Types.ObjectId(meta.doctorId),
            mobile: meta.mobile,
    ...
    // Last attempt: Find appointment created in last 15 minutes with matching sessionId (Account)
    // if (!appt) {
...
export const updateAppointment = async (req, res) => {
  try {
    ...
    if (appt.createdBy !== clerkUserId && appt.owner !== clerkUserId) {
    ...
    if (terminal && body.status && body.status !== appt.status) {
      return res.status(400).json({
        success: false,
        message: "Terminal status cannot be updated",
      });
    }
...
export const getAppointmentsByDoctor = async (req, res) => {
  try {
    ...
    // check before using : doctorId: new mongoose.Types.ObjectId(doctorId)
...
export const getRegisteredUserCount = async (req, res) => {
  try {
    ...
    console.error("Get registered users count error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
};
