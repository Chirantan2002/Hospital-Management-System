...
// TO CREATE THE SERVICE APPOINTMENT
export const createServiceAppointment = async (req, res) => {
  try {
    ...
    console.error("Create Service Appointment unexpected:", err);
...
// TO GET SERVICE BY APPOINTMENT BY ID
export const getServiceAppointmentById = async (req, res) => {
  try {
    const { id } = req.params;
    const appointments = await ServiceAppointment.findById(id).lean();
    if (!appointments) {
      return res.status(404).json({
        success: false,
        message: "Service Appointment not found",
...
// TO UPDATE A PARTICULAR APPOINTMENT BY ID
export const updateServiceAppointment = async (req, res) => {
  try {
    ...
    if (!updated) {
      return res.status(404).json({
        success: false,
        message: "Not updated the service appointment",
      });
    }
...
// TO GET STATS OF SERVICE APPOINTMENTS
export const getserviceAppointmentStats = async (req, res) => {
  try {
    ...
    console.error("Getting stats of service appointment error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
};
