...
// NORMILIZE THE DATA COMING FROM THE DB
const normilizeService = (doc) => {
  if (!doc) {
    return null;
  }
  const id = doc._id || doc.id || String(Math.random()).slice(2);
  const name = doc.name || doc.title || doc.serviceName || "Untitled Service";
  const price = Number(doc.price ?? doc.fee ?? doc.cost ?? doc.amount) || 0;
  const image =
    doc.imageUrl ||
    doc.image ||
    doc.avatar ||
...
const ServiceDashboardPage = ({ servicesProp = null }) => {
  const [services, setServices] = useState(
    Array.isArray(servicesProp) ? servicesProp.map(normilizeService) : [],
  );
  ...
  const fetchServices = async ({ showLoading = true } = {}) => {
    ...
    try {
      ...
      const normalized = (list || []).map(normilizeService).filter(Boolean);
  ...
  useEffect(() => {
    ...
    if (Array.isArray(servicesProp)) {
      setServices(servicesProp.map(normilizeService));
      setLoading(false);
      return () => {
  ...
  const filteredServices = useMemo(() => {
    ...
    return services.filter((s) => {
      ...
      if (!Number.isNaN(qNum) && s.price <= qNum) {
        return true;
      }
      if (s.price.toString().includes(q)) {
        return true;
      }
      return false;
    });
      ...
      <div className={`${sds.innerContainer}`}>
        ...
        <div className={`${sds.search.container}`}>
          <div className={`${sds.search.inputContainer}`}>
            <Search size={16} className="text-[#05668d]" />
            <input
              type="text"
        ...
        {/* TABLE LIST FOR LARGER SCREENS */}
        <div className={`${sds.table.headerLg}`}>
          <div className={`col-span-5`}>Service</div>
          <div className={`col-span-1`}>Price</div>
          <div className={`${sds.table.headerTextLg(1)} col-span-2`}>
            Appointments
          </div>
        ...
        <div className={`${sds.table.body}`}>
          {loading ? (
            <div className={`${sds.states.loading}`}>Loading...</div>
          ) : error ? (
                ...
                <div key={s.id} className={`${sds.table.row}`}>
                  ...
                  <div className={`${sds.table.desktopView}`}>
                    <div className={`col-span-5 flex items-center gap-4`}>
                      ...
                      <h3 className={`${sds.table.desktopServiceName}`}>
                        {s.name}
                      </h3>
                    </div>
