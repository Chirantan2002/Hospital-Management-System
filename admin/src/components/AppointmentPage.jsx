...
const dateTimeFromSlot = (slot) => {
  try {
    const [y, m, d] = slot.date.spit("-");
    ...
    async function load() {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          throw new Error(
            body?.message || `Failed to load appointments (${res.status})`,
          );
        }
  ...
  const filtered = useMemo(() => {
    ...
    return appointments.filter((a) => {
      ...
      return (
        ...
        (a.mobile || "").toLowerCase().includes(a)
      );
    });
  ...
  const adminCancelAppointment = async (id) => {
    ...
    const ok = window.confirm(
      `As admin, mark appointment for ${appt.patientName} with ${
        appt.doctorName
      } on ${formateDateISO(appt.slot.date)} at ${appt.slot.time} as CANCELLED`,
    );
    ...
    try {
      ...
      if (!res.ok) {
        const body = await res.json().catch(() => ({}));
        throw new Error(
          body?.message || `Failed to cancel appointment (${res.status})`,
        );
      }
      ...
      <div className={`${ps.maxWidthContainer}`}>
        <header className={ps.headerContainer}>
          ...
          <div className={`${ps.headerControlsSection}`}>
            <div
              className={`w-auto md:w-full flex flex-col sm:flex-row md:flex-col items-center gap-3`}
            >
              ...
              <div className={`${ps.filterContainer}`}>
                <div className={`${ps.dateFilter}`}>
                  <Calendar size={16} className={`${ps.dateFilterIcon}`} />
                  <input
                    type="date"
