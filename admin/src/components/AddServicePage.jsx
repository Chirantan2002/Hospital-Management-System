...
import { addServiceStyles as ass } from "../assets/dummyStyles";
...
const AddServicePage = ({ serviceId }) => {
  ...
  // i ensures each year is different → no duplicate keys
  const months = [
    "Jan",
    "Feb",
    "Mar",
  ...
  const dayInSelectedMonth = new Date(
    selectedYearNum,
    selectedMonthNum + 1,
    0,
  ).getDate();
  const days = Array.from({ length: dayInSelectedMonth }).map((_, i) =>
    String(i + 1),
  );

  useEffect(() => {
    if (Number(slotDay) > dayInSelectedMonth) {
      setSlotDay(String(dayInSelectedMonth));
    }
  }, [slotMonth, slotYear, dayInSelectedMonth]);
  ...
  return (
    <div className={`${ass.container.main}`}>
      <div className={`${ass.toast.container}`}>
        {toast && (
          <div
            className={`${ass.toast.toastBase} ${
              toast.type === "error"
                ? ass.toast.toastError
                : toast.type === "info"
                  ? ass.toast.toastInfo
                  : ass.toast.toastSuccess
            } animate-slideIn`}
          >
            <div className={`${ass.toast.iconContainer(toast.type)}`}>
              {toast.type === "error" ? (
            ...
            <div className={`flex-1 min-w-0`}>
              <div className={`${ass.toast.title}`}>{toast.title}</div>
              <div className={`${ass.toast.message}`}>{toast.message}</div>
            </div>

            <button
              onClick={() => setToast(null)}
              className={`${ass.buttons.toastClose}`}
            >
      ...
      <form onSubmit={handleSubmit} className={`${ass.container.form}`}>
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 sm:mb-8 gap-4">
          <div>
            <h1 className={`${ass.header.title}`}>
              {serviceId ? "Edit Service" : "Add Service"}
            </h1>
            <p className={`${ass.header.subtitle}`}>
              Create a service card with unique time slots
            </p>
          </div>
          <div className={`${ass.headerActions}`}>
            <button
              type="button"
              onClick={resetForm}
              className={`${ass.buttons.reset}`}
            >
              Reset
            </button>
            <button
        ...
        {/* LEFT SIDE */}
        <div className={`${ass.grids.main}`}>
          <div
            className={`lg:col-span-1 md:col-span-1 col-span-1 flex flex-col items-center`}
          >
            <div className={`${ass.imageUpload.container(errors.image)}`}>
              <div className={`${ass.imageUpload.preview}`}>
                {imagePreview ? (
                  <img
                    src={imagePreview}
                    alt="image-preview"
                  ...
                  <div className={`${ass.imageUpload.placeholder}`}>
                    <Camera className="w-6 h-6" />
                    <span className={`mt-2 text-sm`}>Upload Service Image</span>
                  </div>
                )}
              </div>
              <div className="w-full flex items-center gap-2">
                ...
                <button
                  ...
                  className={`${ass.buttons.uploadImage}`}
                >
                ...
                {(imagePreview || hasExistingImage) && (
                  <button
                    ...
                    className={`${ass.buttons.removeImage}`}
                  >
          ...
          <div className="lg:col-span-2 md:col-span-1 col-span-1 space-y-6">
            <div className={`${ass.grids.formFields}`}>
              <div>
                <label className={`${ass.labels.standard}`}>Service name</label>
                <input
                  value={serviceName}
                  onChange={(e) => setServiceName(e.target.value)}
                  placeholder="e.g. General Consultation"
                  className={`${ass.formFields.input(errors.serviceName)}`}
                />
              </div>

              <div>
                <label className={`${ass.labels.standard}`}>Price</label>
                <input
                  value={Price}
                  onChange={(e) => setPrice(e.target.value)}
                  placeholder="₹ 499"
                  className={`${ass.formFields.input(errors.price)}`}
                  inputMode="numeric"
                />

                <div className="mt-3">
                  <label className={`${ass.labels.standard}`}>
                    Availability
                  </label>
                  <select
                    value={availability}
                    onChange={(e) => setAvailability(e.target.value)}
                    className={`${ass.formFields.select}`}
                  >
            ...
            <div>
              ...
              <textarea
                ...
                rows={4}
                className={`${ass.formFields.textarea(errors.about)}`}
              />
            </div>
            ...
            <div>
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className={`${ass.buttons.addInstruction}`}
                >
              ...
              <div
                className={`${ass.instructions.container(errors.instructions)}`}
              >
                {instructions.map((ins, idx) => (
                  <div
                    key={`${ins}-${idx}`}
                    className={`${ass.instructions.item}`}
                  >
                    <div className={`${ass.icon.number}`}>{idx + 1}.</div>
                    <input
                      value={ins}
                      onChange={(e) => updateInstruction(idx, e.target.value)}
                      placeholder={`Instruction ${idx + 1}`}
                      className={`${ass.instructions.input}`}
                    />
                    {instructions.length > 1 && (
                      <button
                        type="button"
                        onClick={() => removeInstruction(idx)}
                        className={`${ass.instructions.removeButton}`}
                      >
                        <Trash2 className={`${ass.icon.removeInstruction}`} />
                      </button>
                    )}
                  </div>
                ))}
            ...
            {/* slot controls */}
            <div className={`${ass.slots.container(errors.slots)}`}>
              ...
              <div className={`${ass.grids.timeGrid}`}>
                <div className="min-w-0">
                  <label className={`${ass.labels.small}`}>Day</label>
                  <select
                    value={slotDay}
                    onChange={(e) => setSlotDay(e.target.value)}
                    className={`${ass.formFields.smallSelect}`}
                  >
                    {days.map((d) => {
                      const dNum = Number(d);
                      const disabled =
                ...
                <div className="min-w-0">
                  <label className={`${ass.labels.small}`}>Month</label>
                  <select
                    value={slotMonth}
                ...
                <div className="min-w-0">
                  <label className={`${ass.labels.small}`}>Year</label>
                  <select
                    value={slotYear}
                ...
                <div className={`${ass.grids.timeSubGrid}`}>
                  <div className="min-w-0">
                    <label className={`${ass.labels.small}`}>Hour</label>
                    <select
                      value={slotHour}
                      onChange={(e) => setSlotHour(e.target.value)}
                      className={`${ass.formFields.timeSelect}`}
                    >
                      {hours.map((h) => (
                  ...
                  <div className="min-w-0">
                    <label className={`${ass.labels.small}`}>Minute</label>
                    <select
                      value={slotMinute}
                  ...
                  <div className="min-w-0">
                    <label className={`${ass.labels.small}`}>AM/PM</label>
                    <select
                      value={slotAmPm}
                      onChange={(e) => setSlotAmPm(e.target.value)}
                      className={`${ass.formFields.ampmSelect}`}
                    >
                      {ampm.map((a) => (
              ...
              <div>
                ...
                <div className={`${ass.grids.slotsGrid}`}>
                  {slots.length === 0 ? (
                    ...
                    slots.map((s, idx) => (
                      <div key={s} className={`${ass.slots.slotItem}`}>
                        <div className="flex items-center gap-2 min-w-0">
                          <Clock className={`${ass.icon.clock}`} />
                          <div className={`${ass.slots.slotText}`}>{s}</div>
                        </div>
                        <button
                          type="button"
                          onClick={() => removeSlot(idx)}
                          className={`${ass.buttons.slotRemove}`}
                        >
                          <Trash2 className={`${ass.icon.trash}`} />
                        </button>
                      </div>
                    ))
                  )}
